...
public class SlidingTabActivity extends AppCompatActivity implements OnTabSelectListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //注意：数据不用设置setTabData，数据直接跟ViewPager绑定

        tabLayout_1.setViewPager(vp);
        tabLayout_2.setViewPager(vp);
        tabLayout_2.setOnTabSelectListener(this);//点击事件
        ...
        tabLayout_2.showMsg(3, 5);//设置3位置的图标有5这个数据(显示数字用)
        tabLayout_2.setMsgMargin(3, 0, 10);//图标位置
        MsgView rtv_2_3 = tabLayout_2.getMsgView(3);
        if (rtv_2_3 != null) {
            rtv_2_3.setBackgroundColor(Color.parseColor("#6D8FB0"));//图标的颜色
        }

        tabLayout_2.showMsg(5, 5);//设置5数字
